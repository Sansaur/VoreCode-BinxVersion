...
/obj/item/weapon/defibrillator
	...
	icon_state = "defibunit" //ICONS ON VERY BETA -Sansaur
	...
	var/obj/item/weapon/twohanded/paddles
...
/obj/item/weapon/defibrillator/proc/update_overlays()
	overlays.Cut()
	if(!on)
		overlays.Add("[initial(icon_state)]-paddles")
	if(powered)
		overlays.Add("[initial(icon_state)]-powered")
	if(!bcell)
		overlays.Add("[initial(icon_state)]-nocell")
	if(!safety)
		overlays.Add("[initial(icon_state)]-emagged")
...
/obj/item/weapon/defibrillator/proc/update_charge()
	if(powered) //so it doesn't show charge if it's unpowered
		if(bcell)
			...
			overlays.Add("[initial(icon_state)]-charge[ratio]")
...
/obj/item/weapon/defibrillator/emp_act(severity)
	...
	if(safety)
		...
		playsound(get_turf(src), 'sound/machines/windowdoor.ogg', 50, 0)
	else
		safety = 1
	...
	spawn(50)
		if(bcell)
			if(bcell.charge >= paddles.revivecost)
				...
				playsound(get_turf(src), 'sound/machines/click.ogg', 80, 0)
			else
...
/obj/item/weapon/twohanded/shockpaddles/suicide_act(mob/user)
	...
	playsound(get_turf(src), 'sound/machines/chime.ogg', 50, 1, -1)
	return (OXYLOSS)
...
/obj/item/weapon/twohanded/shockpaddles/attack(mob/M, mob/user) //Cuidado con esto - Sansaur
	var/halfwaycritdeath = (-50 + -100) / 2
		...
		if(do_after(user, 30, target = M))
			...
			if(do_after(user, 30, target = M)) //Takes longer due to overcharging
				...
				if(M && M.stat == DEAD)
					...
					playsound(get_turf(src), 'sound/machines/emitter2.ogg', 50, 0)
					busy = 0
					update_icon()
					return
				...
				playsound(get_turf(src), 'sound/machines/chime.ogg', 100, 1, -1)
				playsound(loc, 'sound/weapons/chime.ogg', 100, 1, -1)
...
/obj/item/weapon/twohanded/paddles
	name = "defibrillator paddles"
	desc = "CLEAR!!"
	icon = 'icons/obj/weapons.dmi'
	icon_state = "electric_hand"
...
/obj/item/weapon/defibrillator/proc/deductcharge(var/amount) //starts without a cell for rnd
	if(bcell)
		bcell.use(amount)
...
/obj/item/weapon/defibrillator/proc/update_power() //Used on update_icon()
	if(bcell)
		if(bcell.charge < paddles.revivecost)
			powered = 0
		else
...
/obj/item/weapon/defibrillator/proc/update_charge() //Used on update_icon()
	if(powered) //so it doesn't show charge if it's unpowered
		if(bcell)
			var/ratio = bcell.charge / bcell.maxcharge
			ratio = Ceiling(ratio*4) * 25
...
/obj/item/weapon/defibrillator/attack_hand(mob/user) //BUGEADO
	if(loc == user)
		if(slot_flags == SLOT_BACK)
			if(user.get_item_by_slot(slot_back) == src)
				toggle_paddles()
...
/obj/item/weapon/defibrillator/MouseDrop(obj/over_object) //Dragging the defibrilator off the backpack
	if(ismob(src.loc))
		var/mob/M = src.loc
		//var/obj/screen/inventory/H = over_object //Be careful with this -Sansaur
		if(!M.unEquip(src))
			return
			M.put_in_hands(src) //This can break too, before it was M.put_in_hands(src, H.held_index) -Sansaur

		//if(istype(over_object, /obj/screen/inventory/slot_r_hand)) We must fix this??



/obj/item/weapon/defibrillator/attackby(obj/item/weapon/W, mob/user, params)
	if(W == paddles)
		//paddles.unwield() //Fix this later
		toggle_paddles()
		...
		if(bcell)
			...
			if(C.maxcharge < paddles.revivecost)
				user << "<span class='notice'>[src] requires a higher capacity cell.</span>"
				return
			//if(!user.unEquip(W)) This must be fixed, I don't know what it is.
				//return
			W.loc = src
			bcell = W
			user << "<span class='notice'>You install a cell in [src].</span>"
...
/obj/item/weapon/defibrillator/emp_act(severity)
	if(bcell)
		bcell.use(1000 / severity) //Changed from "Deductcharge" to "use" -Sansaur
	if(safety)
		safety = 0
		src.visible_message("<span class='notice'>[src] beeps: Safety protocols disabled!</span>")
		playsound(get_turf(src), 'sound/machines/windowdoor.ogg', 50, 0)
...
/obj/item/weapon/defibrillator/equipped(mob/user, slot)
	...
	if((slot_flags == SLOT_BACK && slot != slot_back) || (slot_flags == SLOT_BELT && slot != slot_belt))
		...
		update_icon()
/*
/obj/item/weapon/material/twohanded/defibrillator/item_action_slot_check(slot, mob/user)
	if(slot == user.getBackSlot())
		return 1
*/
...
/obj/item/weapon/defibrillator/defibrillator/Destroy()
	if(on)
		var/M = get(paddles, /mob)
		remove_paddles(M)
	. = ..()
...
//WE AREN'T GOING TO MAKE THEM WIELDED ANYMORE UNTIL WE FIX IT.
/obj/item/weapon/shockpaddles
	...
	throwforce = 6
	w_class = 4
	//flags = NO_DROP haven't found a NO_DROP flag, instead we'll use a "you cannot drop this" on dropped()

	//wielded = 0 //We shouldn't want them to start wielded
	var/revivecost = 1000
	var/cooldown = 0
	var/busy = 0
	var/obj/item/weapon/defibrillator/defib
...
/obj/item/weapon/shockpaddles/dropped(mob/user)
	if(!req_defib)
		return ..()
	if(user)
		//var/obj/item/weapon/twohanded/offhand/O = user.get_inactive_held_item()
...
/obj/item/weapon/shockpaddles/proc/recharge(var/time)
	if(req_defib || !time)
		return
	cooldown = 1
	update_icon()
...
//ALL THE DEFIBRILATOR SOUNDS ARE BROKEN BECAUSE I'M NOT GOING TO SEARCH FOR SOUNDS RIGHT NOW.
...
/* We must find a way to fix this -Sansaur
/obj/item/weapon/material/twohanded/shockpaddles/dropped(mob/user)
	if(!req_defib)
		return ..()
	if(user)
		var/obj/item/weapon/twohanded/offhand/O = user.get_inactive_held_item()
	...
	return unwield(user)
*/
/obj/item/weapon/shockpaddles/proc/check_defib_exists(mainunit, mob/living/carbon/human/M, obj/O)
	if(!req_defib)
		return 1 //If it doesn't need a defib, just say it exists
	if (!mainunit || !istype(mainunit, /obj/item/weapon/defibrillator))	//To avoid weird issues from admin spawns
		M.unEquip(O)
...
//Be careful with these, there were some DEFINED numbers here, but it didn't get them properly, they were the soft-crit-limit and the death limit - Sansaur
/obj/item/weapon/shockpaddles/attack(mob/M, mob/user)
	var/halfwaycritdeath = (-50 + -100) / 2
	...
	if(user.zone_sel.selecting != BP_TORSO) //Check mob_helpers.dm to see bodyparts
		user << "<span class='warning'>You need to target your patient's \
			chest with [src]!</span>"
		return
	...
	if(user.a_intent == "harm")
		...
		if(do_after(user, 30, target = M))
			...
			if(do_after(user, 30, target = M)) //Takes longer due to overcharging
				...
				user.visible_message("<span class='boldannounce'><i>[user] shocks [M] with \the [src]!</span>", "<span class='warning'>You shock [M] with \the [src]!</span>")
				playsound(get_turf(src), 'sound/machines/chime.ogg', 100, 1, -1)
				playsound(loc, 'sound/weapons/emitter2.ogg', 100, 1, -1)
				var/mob/living/carbon/human/HU = M
				M.emote("scream")
				if(M)
					M.Weaken(1)
	...
	if(do_after(user, 30, target = M)) //beginning to place the paddles on patient's chest to allow some time for people to move away to stop the process
		...
		if(do_after(user, 20, target = M)) //placed on chest and short delay to shock for dramatic effect, revive time is 5sec total
			...
			if(H.stat == DEAD)
				...
				if (H.suiciding || (H.disabilities & NOCLONE))
					failed = "<span class='warning'>[req_defib ? "[defib]" : "[src]"] buzzes: Resuscitation failed - Recovery of patient impossible. Further attempts futile.</span>"
				//else if (H.hellbound) fix this later
					//failed = "<span class='warning'>[req_defib ? "[defib]" : "[src]"] buzzes: Resuscitation failed - Patient's soul appears to be on another plane of existance.  Further attempts futile.</span>"
				else if (tplus > tlimit)
					failed = "<span class='warning'>[req_defib ? "[defib]" : "[src]"] buzzes: Resuscitation failed - Body has decayed for too long. Further attempts futile.</span>"
				else if (!(/obj/item/organ/internal/heart in H.get_organ(BP_TORSO))) //GET_ORGAN RETUNS A LIST OF ORGAN IN A BODYPART
					failed = "<span class='warning'>[req_defib ? "[defib]" : "[src]"] buzzes: Resuscitation failed - Patient's heart is missing.</span>"
				else if(total_burn >= 200 || total_brute >= 200)
					failed = "<span class='warning'>[req_defib ? "[defib]" : "[src]"] buzzes: Resuscitation failed - Severe tissue damage makes recovery of patient impossible via defibrillator. Further attempts futile.</span>"
				else if(H.stat == DEAD || istype(src, /mob/new_player))
					failed = "<span class='warning'>[req_defib ? "[defib]" : "[src]"] buzzes: Resuscitation failed - No activity in patient's brain. Further attempts may be successful.</span>"
				else
					var/obj/item/organ/internal/brain/BR = H.get_organ(/obj/item/organ/internal/brain)
					if(!BR) // BR.damaged_brain, we need to check if brain is in or if it has decayed
						failed = "<span class='warning'>[req_defib ? "[defib]" : "[src]"] buzzes: Resuscitation failed - Patient's brain is missing or damaged beyond point of no return. Further attempts futile.</span>"
			...
			if (!(/obj/item/organ/internal/heart in H.get_organ(BP_TORSO)))
				user.visible_message("<span class='warning'>[req_defib ? "[defib]" : "[src]"] buzzes: Patient's heart is missing. Operation aborted.</span>")
				playsound(get_turf(src), 'sound/machines/chime.ogg', 50, 0)
